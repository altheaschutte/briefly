...
SUPABASE_POSTGRES_URL="postgresql://postgres:password@localhost:54322/postgres"
# Optional overrides:
...
# Apply Supabase migrations (in ../backend/supabase/migrations) so the pgvector
# table/index for semantic recall (`memory_messages`) exists before running dev.
```

The Producer agent stores/retrieves semantic recall embeddings in Supabase (pgvector). Ensure your local Supabase instance is reachable via `SUPABASE_DB_URL` before running the app.
