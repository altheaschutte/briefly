    ...
    @State private var dragPreviewProgress: Double?
	    ...
	    @State private var navigateToDiveDeeperEpisode: Bool = false
		    ...
		    init(episodeId: UUID,
                 ...
                 onCreateEpisode: (() -> Void)? = nil,
                 usesCustomChrome: Bool = false,
	    ...
	    init(episode: Episode,
             ...
             usesCustomChrome: Bool = false,
             onScrollOffsetChange: ((CGFloat) -> Void)? = nil) {
	        self.init(episodeId: episode.id,
		            ...
		            NavigationLink(
		                destination: Group {
		                    if let createdDiveDeeperEpisode {
		                        EpisodeDetailView(episode: createdDiveDeeperEpisode,
                                              service: service,
                                              onCreateEpisode: onCreateEpisode,
                                              onScrollOffsetChange: onScrollOffsetChange)
		                    } else {
...
//                    actionRow
        ...
        .navigationBarTitleDisplayMode(.inline)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarLeading) {
                    headerIconButton(
                        systemName: usesCustomChrome ? "chevron.down" : "chevron.left",
                        accessibilityLabel: usesCustomChrome ? "Close episode" : "Back",
                        action: close
                    )
                }
        ...
        .sheet(isPresented: $showSpeedSheet) {
            PlaybackSpeedSheet(selectedSpeed: audioManager.playbackSpeed, style: .dark) { speed in
...
private extension EpisodeDetailView {
    ...
    private var chromeForeground: Color { .brieflyTabBarActive }
    private var chromeBorder: Color { chromeForeground.opacity(0.2) }
    private var sectionTopPadding: CGFloat { 12 }
    private var sectionHeaderBottomPadding: CGFloat { 6 }

    private var headerFadeOverlay: some View {
...
//    }
    ...
    func loadDetailsIfNeeded(force: Bool = false) async {
        ...
        do {
            let latest = try await service.fetchEpisode(id: episodeId)
            applyEpisode(latest)
            errorMessage = nil
            hasLoaded = true
        } catch {
    ...
    private func createDiveDeeperEpisode(seed: SegmentDiveDeeperSeed) async {
        ...
        do {
            let creation = try await service.requestDiveDeeperEpisode(
                parentEpisodeID: detailedEpisode.id,
                seedID: seed.id,
                targetDurationMinutes: nil
            )
            requestedDiveDeeperSeedIDs.insert(seed.id)
            let created = try await service.fetchEpisode(id: creation.episodeId)
    ...
    func downloadEpisode() {
        ...
        Task {
            ...
            if let direct = detailedEpisode.audioURL {
                ...
                audioURL = await service.fetchSignedAudioURL(for: detailedEpisode.id)
            }
    ...
    func openMail(subject: String) {
	    ...
	    func deleteEpisode() async {
	        ...
	        do {
	            try await service.deleteEpisode(id: detailedEpisode.id)
	            await MainActor.run {
	                if audioManager.currentEpisode?.id == detailedEpisode.id {
	                    audioManager.stop()
	                }
...
private final class SharedImageCache {
...
private struct CachedAsyncImage<Content: View, Placeholder: View, Failure: View>: View {
    let url: URL?
